...
export default function PostPage({ frontmatter:{title,date,type,cover_image  },slug , content , posts}) {
 ...
 const previousSlug = currentIndex >= 1 ? allBlogPaths[currentIndex - 1]?.slug : null;
const nextSlug = currentIndex < allBlogPaths.length - 1 ? allBlogPaths[currentIndex + 1]?.slug : null;
  ...
  return (
    <div className="grid md:grid-cols-[auto_20rem] p-10">
      ...
      <div className="post-body prose" dangerouslySetInnerHTML={{ __html: htmlContent }}></div>
    ...
    <div className="px-6">
      ...
      <p className='font-bold py-1 text-xl border-b-[0.1rem] before:content-[""] relative before:absolute before:bottom-0 before:border-b-indigo-500 before:border-b-2  before:w-12  '>Latest Blog</p>
   ...
   <div>
    ...
    <div>
        ...
        <h1 className="text-2xl font-bold  py-4 border-b-2 border-black">Related Blog</h1>
            ...
            allBlogPaths.map((el,index)=>{
              return(
               <div key={index} className=" relative h-[15rem]">
                 <h1 className="absolute bottom-0 bg-indigo-500 m-2 text-sm rounded-md text-white font-bold px-4 py-2 text-center">{el.frontmatter.title}</h1>

                 <img className="w-full h-full " src={el.frontmatter.cover_image} />
...
export async function getStaticProps({ params }) {
  ...
  return {
    props: {
     frontmatter,slug, content  ,posts
    },
  };
